...
def filesPost():
    if request.cookies.get('key'):
        DB = mysql.connector.connect(
            host="127.0.0.1", user="root", passwd="DonAcDum7557")
        cursor = DB.cursor()
        cursor.execute("USE userinfo")
        print(cursor.execute("SELECT * FROM tokens WHERE token='" +
        ...
        if (files.__sizeof__() > 20):
            ...
            if (files.__sizeof__() > 20):
                for i in range(0, 4):
                    s += [item[i] for item in files]
                q = os.listdir(
                    "app/static/../../../../uploads/" + s[1].replace('/', 'a'))
                q = [base64.b64decode(i.split('!')[0].split('.')[
                                      0]).decode() for i in q]
...
def fileDownload(pathh, file):
    if (SecureFile(file)):
        if (len(base64.b64decode(file).decode().split('.')) != 1):
            nam = file + '.' + base64.b64decode(file).decode().split('.')[-1]
        else:
            nam = file
        print(nam)
        return make_response(flask.send_from_directory(pathh, nam, as_attachment=True))
    else:
...
def indexPost():
    if request.method == 'POST' and SecureStr(request.form["login"]):
        login = pbkdf2.crypt(
            request.form["login"], iterations=150, salt="NotASalt")
        password = pbkdf2.crypt(
            request.form["password"], iterations=150, salt="NotASalt")
        DB = mysql.connector.connect(
            host="127.0.0.1", user="root", passwd="DonAcDum7557")
        cursor = DB.cursor()
        cursor.execute("USE userinfo")
        ...
        if len(str(files)) < 20:
            ...
            os.mkdir(app.config['UPLOAD_FOLDER'] + pbkdf2.crypt(login,
                                                                iterations=150, salt="NotASalt").replace('/', 'a'))
            print(app.config['UPLOAD_FOLDER'] + pbkdf2.crypt(login,
                                                             iterations=150, salt="NotASalt").replace('/', 'a'))
            ...
            return render_template('registration.html',
                                   title='Sign in', message="User with this login already exist")
    else:
        return render_template('registration.html', title='Sign in')
...
def loginPost():
    if request.method == 'POST' and SecureStr(request.form["login"]):
        ...
        s = []
        files = cursor.fetchall()
        if (str(files).__len__() > 3 and files[0][2] == password):
            for i in range(0, 3):
                s += [item[i] for item in files]
            timee = time.time()
            key = pbkdf2.crypt(str(login + password + str(timee)),
                               iterations=150, salt="NotASalt")
            ...
            return render_template(Lname, title='Sign in', message="Login or password is incorrect")
        cursor.close()
        conn.close()
    else:
    ...
    if file:
        ...
        if base64.b64encode(bytes(filename, 'utf-8')).decode() + '.' + filename.split('.')[-1] in dirs:
            ...
            if (len(filename.split('.')) != 1):
                ...
                nam = ''
            print(nam)
            open(os.path.join(app.config['UPLOAD_FOLDER'] + idd + '/' + base64.b64encode(bytes(
                filename, 'utf-8')).decode() + nam), 'wb').writelines(s)
...
def index():
    if (request.cookies.get('key')):
        ...
        cursor = DB.cursor()
        cursor.execute("USE userinfo")
        cursor.execute("SELECT * FROM tokens WHERE token='" +
                       request.cookies.get("key") + "'")
        ...
        if (files.__sizeof__() > 20):
            ...
            if (files.__sizeof__() > 20):
                ...
                if request.method == 'POST':
                    return file_upload("",  s[1].replace('/', 'a') + "/", free_space=s[4] * 1048576 - get_size("../../uploads/" + s[1]))
                t = ''
                ...
                r = make_response(render_template(
                    "account.html", tittle="Account", login=str(s[3]), space=space, t=t))
                return r
            else:
                return redirect('/')
        else:
        ...
        if (files.__sizeof__() > 20):
            ...
            if (files.__sizeof__() > 20):
                ...
                if 'text' in request.form:
                    f = open('app/' + path + base64.b64encode(bytes(str(request.args.get("file")),
                                                                    'utf-8')).decode() + '.' + request.args.get("file").split('.')[-1], 'w')
                ...
                if (t in image_types):
                    type = 'i'
                    file = path + base64.b64encode(bytes(str(request.args.get("file")), 'utf-8')).decode(
                    ) + '.' + request.args.get("file").split('.')[-1]
                elif (t in txt_types):
                    type = 't'
                    file = [i for i in open('app/' + path + base64.b64encode(bytes(str(request.args.get(
                        "file")), 'utf-8')).decode() + '.' + request.args.get("file").split('.')[-1], 'r').readlines()]
                else:
        ...
        if not request.args.get("delete") is None:
            ...
            if (files.__sizeof__() > 20):
                for i in range(0, 4):
                    s += [item[i] for item in files]
                cursor.execute(
                    "SELECT * FROM users WHERE id='" + str(s[3]) + "';")
                s = []
                files = cursor.fetchall()
                if (files.__sizeof__() > 20):
                    for i in range(0, 4):
                    ...
                    if (1 != len(dele.split('.'))):
                        dele = path + base64.b64encode(bytes(str(dele), 'utf-8')).decode() + '.' + dele.split('.')[-1]
                    else:
                        dele = path + base64.b64encode(bytes(str(dele), 'utf-8')).decode()
                    print(dele)
                    os.remove(dele)
        if not request.args.get("file") is None:
            DB = mysql.connector.connect(
